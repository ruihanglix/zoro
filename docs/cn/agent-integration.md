...
对于能执行 shell 命令的 Agent，`zoro` CLI 提供了最便捷的文献库访问方式。与文件系统方式（需要解析 JSON 文件）或 MCP 服务器（需要 MCP 协议支持）不同，CLI 适用于**任何能执行 shell 命令的 Agent**。
...
`--json` 参数输出机器可读的 JSON，方便通过 `jq` 处理或程序化解析。详见 [Zoro CLI](cli.md) 完整文档。
