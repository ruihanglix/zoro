...
pub async fn plugin_ai_chat(
    ...
    let client = zoro_ai::client::ChatClient::new(state.http_client.clone(), &base_url, &api_key, &model, resolved.resolved_format);
...
pub async fn plugin_ai_chat_stream(
    ...
    tokio::spawn(async move {
        let client = zoro_ai::streaming::StreamingClient::new(http_client, &base_url, &api_key, &model, resolved.resolved_format);
