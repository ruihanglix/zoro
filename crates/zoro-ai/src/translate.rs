...
pub async fn translate_text(
    ...
    let client = ChatClient::new(client.clone(), &config.base_url, &config.api_key, &config.model);
...
pub async fn extract_glossary_terms(
    ...
    let client = ChatClient::new(http_client.clone(), &config.base_url, &config.api_key, &config.model);
