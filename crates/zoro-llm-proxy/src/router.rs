...
impl Router {
    ...
    pub fn select<'a>(
        ...
        // Build list of all healthy providers (for fallback)
        ...
        match strategy {
            ...
            RoutingStrategy::RoundRobin => {
                ...
                let idx = self.round_robin_counter.fetch_add(1, Ordering::Relaxed) % all_healthy.len();
    ...
    pub fn select_retry<'a>(
        ...
        match strategy {
            ...
            RoutingStrategy::Auto | RoutingStrategy::RoundRobin => {
                ...
                if let Some(p) = eligible.iter().find(|p| p.models.contains(&requested_model.to_string())) {
                    return Some(RouteTarget {
                        provider: p,
                        model: requested_model.to_string(),
                    });
