...
# Build the zoro-cli sidecar binary and copy it to src-tauri/binaries/
# with the Tauri-required target-triple suffix.
#
# Usage:
#   ./scripts/build-cli-sidecar.sh [--release] [--target <triple>]
...
DEST="$BINARIES_DIR/zoro-${TARGET_TRIPLE}${EXT}"
...
if [ "$TARGET_TRIPLE" = "universal-apple-darwin" ]; then
  ARM_BIN="$BINARIES_DIR/zoro-aarch64-apple-darwin"
  X86_BIN="$BINARIES_DIR/zoro-x86_64-apple-darwin"
...
echo "Building zoro-cli for $TARGET_TRIPLE ..."
cargo build -p zoro-cli $RELEASE_FLAG --target "$TARGET_TRIPLE"
